...
-(id) init
{
	...
	if( (self=[super init]) ) {
        ...
        CCLabelTTF * label1 = [CCLabelTTF labelWithString:@"fade" fontName:@"Marker Felt" fontSize:30];
        ...
        CCMenuItemFont * item = [CCMenuItemFont itemWithString:@"action" block:^(id sender){
            ...
            id fade;
            if (label1.tag == 0) {
                fade = [CCFadeOut actionWithDuration:2];
                label1.tag = 1;
            }
            else{
                fade = [CCFadeIn actionWithDuration:2];
                label1.tag = 0;
            }
            [label1 runAction:fade];
        }];
